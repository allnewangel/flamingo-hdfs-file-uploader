...
/**
 * 리소스를 처리하는 유틸리티 클래스. 이 유틸리티를 클래스를 이용해서 해당 리소스에 대한 URL 및 파일 정보를 얻어올 수 있다.
 *
 * @author Edward KIM
 * @since 0.1
 */
public class ResourceUtils {
    ...
    private static ClassLoader getDefaultClassLoader() {
        ...
        try {
            ...
            e.printStackTrace();
            System.out.println("쓰레드의 컨텍스트 클래스 로더에 접근할 수 없습니다. 시스템 클래스 로더를 사용합니다.");
        }
        if (cl == null) {
    ...
    public static File getFile(URL resourceUrl, String description) throws FileNotFoundException {
        if (!URL_PROTOCOL_FILE.equals(resourceUrl.getProtocol())) {
            throw new FileNotFoundException(description + "은 절대 파일 경로가 아닙니다. 왜냐하면 파일 시스템에 존재하지 않기 때문입니다 : " + resourceUrl);
        }
